...
@section('header_title', 'دەستکاریکردنی پاکێج')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
            <h2 class="text-xl font-bold text-slate-800">دەستکاریکردنی پاکێج و ئۆفەر</h2>
            <p class="text-xs text-slate-500 mt-1">زانیارییەکان، نرخ، پشکنینەکان و وەسفی پاکێجەکە نوێبکەرەوە.</p>
        </div>
    ...
    <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
        <form action="{{ route('lab.packages.update', $package) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 font-bold text-xs text-slate-800 outline-none" 
                           id="name" type="text" name="name" value="{{ old('name', $package->name) }}" placeholder="نموونە: پاکێجی پشکنینی تەواوی جەستە" required>
                </div>
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 font-bold text-xs text-slate-800 outline-none" 
                           id="name_ar" type="text" name="name_ar" value="{{ old('name_ar', $package->name_ar) }}" dir="rtl">
                </div>
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 font-bold text-xs text-slate-800 outline-none" 
                           id="name_en" type="text" name="name_en" value="{{ old('name_en', $package->name_en) }}" dir="ltr">
                </div>
            </div>
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-2">
                    پشکنینە هاوپێچەکان لەناو ئەم پاکێجەدا
                </label>
                <div class="p-4 bg-slate-50 border border-slate-200/70 rounded-2xl max-h-64 overflow-y-auto space-y-2">
                    @php $selectedIds = is_array($package->test_ids) ? $package->test_ids : []; @endphp
                        ...
                        <label class="flex items-center justify-between p-2.5 bg-white rounded-xl border border-slate-100 hover:border-purple-200 cursor-pointer transition-all">
                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="test_ids[]" value="{{ $test->id }}" 
                                       ...
                                       {{ in_array($test->id, $selectedIds) ? 'checked' : '' }}
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 font-bold text-xs text-slate-800 outline-none" 
                           id="original_price" type="text" inputmode="numeric" name="original_price" value="{{ old('original_price', $package->original_price ? number_format($package->original_price) : '') }}" placeholder="نموونە: 60,000" oninput="formatPriceInput(this); updateDiscountPercent();">
                </div>
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 font-bold text-xs text-slate-800 outline-none" 
                           id="price" type="text" inputmode="numeric" name="price" value="{{ old('price', $package->price ? number_format($package->price) : '') }}" placeholder="نموونە: 42,000" oninput="formatPriceInput(this); updateDiscountPercent();" required>
                </div>
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-purple-50/60 border border-purple-200 text-purple-700 rounded-xl font-black text-xs outline-none cursor-not-allowed" 
                           id="discount_display" type="text" value="{{ $package->discount ?? 0 }}% ئۆفەر" readonly>
                </div>
            </div>
            ...
            <div class="space-y-4 pt-2">
                ...
                <input type="hidden" id="description_en" name="description_en" value="{{ old('description_en', $package->description_en) }}">
            </div>

            <!-- Active Status -->
            <div class="p-4 bg-slate-50 rounded-xl border border-slate-200/60">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="w-4 h-4 text-purple-600 rounded border-slate-300 focus:ring-purple-500 cursor-pointer" {{ old('is_active', $package->is_active) ? 'checked' : '' }}>
