                    ...
                    @forelse($orders as $order)
                        ...
                        <tr>
                            ...
                            <td style="text-align: center;">
                                <button type="button" onclick="openOrderModal({{ json_encode($order) }}, {{ json_encode($details) }}, {{ json_encode($loc) }}, {{ json_encode($order->items) }}, '{{ $paymentMethodKurdish }}', '{{ $mapUrl }}')" 
...
<!-- Detailed Form Modal with All Comprehensive Details -->
<div id="orderModal" style="position: fixed; inset: 0; z-index: 999; background: rgba(15,23,42,0.65); backdrop-filter: blur(5px); display: none; align-items: center; justify-content: center; padding: 16px;">
    <div style="background: #ffffff; border-radius: 24px; max-width: 680px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); padding: 26px; direction: rtl; text-align: right; border: 1px solid #e2e8f0;">
        <!-- Modal Header -->
...
@section('scripts')
<script>
function openOrderModal(order, details, loc, items, paymentMethodKurdish, mapUrl) {
    ...
    badge.textContent = `#ORD-${order.id}`;
    subtitle.textContent = `بەروار و کاتی تۆمارکردن: ${order.created_at}`;
    ...
    const notes = details.notes || details.description || 'هیچ تێبینییەک نەنوسراوە';
    ...
    content.innerHTML = `
        ...
        <div style="background: #eff6ff; padding: 16px; border-radius: 16px; border: 1px solid #dbeafe;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 12px;">
                <div>
                    ...
                    <strong style="color: #0f172a; font-size: 0.92rem;">${patientName}</strong>
                </div>
                <div>
                    <span style="font-size: 0.72rem; color: #64748b; display: block;">ژمارەی مۆبایل:</span>
                    <a href="tel:${patientPhone}" style="color: #2563eb; font-weight: 800; text-decoration: none; font-size: 0.9rem;" dir="ltr">📞 ${patientPhone}</a>
                </div>
                <div>
        ...
        <div style="background: #f8fafc; padding: 16px; border-radius: 16px; border: 1px solid #e2e8f0;">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px;">
                <span style="font-size: 0.78rem; font-weight: 800; color: #334155;">📍 شێوازی وەرگرتن & لۆکەیشن</span>
                <a href="${mapUrl}" target="_blank" style="background: #dc2626; color: #ffffff; padding: 4px 10px; border-radius: 8px; font-size: 0.72rem; font-weight: 800; text-decoration: none; display: inline-flex; align-items: center; gap: 4px;">
                    📍 بینین لەسەر نەخشەی گووگڵ ↗
                </a>
            </div>
            <div style="font-size: 0.82rem; color: #334155; line-height: 1.7;">
                <div><strong>شێوازی وەرگرتن:</strong> <span style="color: #7e22ce; font-weight: 800;">${collectionMethod}</span></div>
                <div><strong>ناونیشان:</strong> ${address}</div>
                <div><strong>تێبینی نەخۆش:</strong> <span style="color: #64748b;">${notes}</span></div>
            </div>
        </div>
        ...
        <div style="background: #f8fafc; padding: 16px; border-radius: 16px; border: 1px solid #e2e8f0;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 12px; font-size: 0.82rem;">
                <div>
                    ...
                    <span style="color: #059669; font-weight: 800;">💵 ${paymentMethodKurdish}</span>
                </div>
                <div>
                    <span style="font-size: 0.72rem; color: #64748b; display: block;">کۆی گشتی نرخ:</span>
                    <strong style="color: #059669; font-size: 1rem;" dir="ltr">${Number(order.total_price).toLocaleString()} IQD</strong>
                </div>
                <div>
                    <span style="font-size: 0.72rem; color: #64748b; display: block;">بارودۆخ:</span>
                    <span style="color: #2563eb; font-weight: 800;">${order.status}</span>
                </div>
            </div>
        </div>
