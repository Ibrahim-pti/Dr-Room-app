...
@section('header_title', 'دەستکاریکردنی پشکنین')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">زانیاری، نرخ و وەسفی پشکنینەکە نوێبکەرەوە یان کلیک لە وەرگێڕان بکە بۆ وەرگێڕانی خۆکار.</p>
        </div>
        ...
        <form action="{{ route('lab.tests.update', $test) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-bold text-xs text-slate-800 outline-none" 
                           id="name" type="text" name="name" value="{{ old('name', $test->name) }}" placeholder="نموونە: پشکنینی گشتی خوێن (CBC)" required>
                </div>
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-bold text-xs text-slate-800 outline-none" 
                           id="name_ar" type="text" name="name_ar" value="{{ old('name_ar', $test->name_ar) }}" dir="rtl">
                </div>
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-bold text-xs text-slate-800 outline-none" 
                           id="name_en" type="text" name="name_en" value="{{ old('name_en', $test->name_en) }}" dir="ltr">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-bold text-xs text-slate-800 outline-none" 
                           id="price" type="text" inputmode="numeric" name="price" value="{{ old('price') ? number_format((float)str_replace(',', '', old('price'))) : ($test->price ? number_format($test->price) : '') }}" placeholder="نموونە: 10,000" oninput="formatPriceInput(this)" required>
                </div>
                <div>
                    ...
                    <input class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-bold text-xs text-slate-800 outline-none" 
                           id="discount" type="number" min="0" max="100" name="discount" value="{{ old('discount', $test->discount) }}">
                </div>
            </div>
            ...
            <div class="p-4 bg-slate-50 rounded-xl border border-slate-200/60">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="w-4 h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500 cursor-pointer" {{ old('is_active', $test->is_active) ? 'checked' : '' }}>
                    <span class="text-xs text-slate-800 font-bold">ئەم پشکنینە چالاکە و لە ئەپڵیکەیشن بۆ نەخۆش بەردەست دەبێت</span>
                </label>
            </div>
