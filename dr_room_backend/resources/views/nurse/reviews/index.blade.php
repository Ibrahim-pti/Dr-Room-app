...
@section('content')
<div class="mb-6">
    <h2 class="text-xl font-bold text-slate-800">هەڵسەنگاندن و فیدباکەکان</h2>
    <p class="text-sm text-slate-500 mt-1">ئەوەی نەخۆشەکانت دەربارەی خزمەتگوزارییەکانت و ئەزموونیان نووسیویانە.</p>
</div>

@php
    $total = $nurse?->total_reviews ?? 0;
    $average = (float) ($nurse?->rating ?? 0);
@endphp
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200/60 p-6 mb-6">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
        <div class="text-center md:border-l md:border-slate-200">
            ...
            <p class="text-sm text-slate-500 mt-2">لەسەر بنەمای {{ $total }} هەڵسەنگاندن</p>
        </div>
        ...
        <div class="p-5 {{ !$loop->last ? 'border-b border-slate-100' : '' }}">
            <div class="flex items-start justify-between gap-4">
                <div class="flex items-start gap-3">
                    <div class="w-10 h-10 rounded-full bg-teal-50 text-teal-600 flex items-center justify-center font-bold shrink-0">
            ...
            @if($review->comment)
                <p class="mt-3 text-slate-600 text-sm leading-relaxed pr-13">{{ $review->comment }}</p>
            @endif
        </div>
    @empty
        <div class="p-12 text-center text-slate-400">
            ...
            <p class="font-medium text-slate-600">هیچ هەڵسەنگاندنێک نییە هێشتا</p>
            <p class="text-xs mt-1">کاتێک نەخۆشەکان فیدباک بدەن، لێرەدا دەردەکەون.</p>
        </div>
    @endforelse
</div>
